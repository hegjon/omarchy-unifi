...
// The Ubiquiti "U" mark, drawn as a vector path so it takes the bar's own
// foreground colour like a font glyph would. The path is the 24×24 outline
// from assets/ubiquiti.svg; see assets/NOTICE for its provenance.
Item {
  id: root
  ...
  // Multisampling goes on this item, not the Shape: a layer snapshots an item
  // at its own size before its transform, so a layered 16px Shape holding a
  // 24-unit path was clipped to the left two-thirds of the mark.
  layer.enabled: true
  layer.samples: 4

  Shape {
    // The source is 24 units square; draw it at that size and scale the whole
    // thing down to the box we were given.
    width: 24
    height: 24
    antialiasing: true
